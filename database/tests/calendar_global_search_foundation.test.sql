...
select has_function('private', 'prevent_calendar_history_mutation', array[]::text[], 'append-only calendar history guard exists');
...
select ok(exists (
  ...
  where conrelid = 'public.calendar_events'::regclass
    and conname = 'calendar_events_scope_target_valid'
), 'calendar scope target is constrained');
select ok(exists (
  select 1 from pg_constraint
  where conrelid = 'public.calendar_events'::regclass
    and conname = 'calendar_events_recurrence_valid'
), 'calendar recurrence is constrained');
select ok(exists (
  select 1 from pg_constraint
  where conrelid = 'public.calendar_event_attendees'::regclass
    and conname = 'calendar_attendees_response_valid'
), 'attendee response state is constrained');
